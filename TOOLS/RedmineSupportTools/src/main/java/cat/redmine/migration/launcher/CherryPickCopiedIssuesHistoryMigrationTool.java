...
/**
* 설명 : 레드마인 프로젝트 복제 후 기존 프로젝트의 특점 일감이 기 복제된 상태에서 해당 일감에 대한 이력만을 새 프로젝트로 복제해 주는 도구
* 작성자 : 전길수
* 작성일 : 2022.7.13.
* 변경일 : 2022.7.13.
*/
public class CherryPickCopiedIssuesHistoryMigrationTool {
		/**
	* @param args
	*/
	...
	*/	
	private static int copyIssueIntoTargetProject(Connection conn, Issue issue)  throws SQLException{
		...
		try{
			...
			pstmtIns.setInt(1, issue.getTrackerId());
			pstmtIns.setInt(2, issue.getProjectId());
			pstmtIns.setString(3, issue.getSubject());
			pstmtIns.setString(4, issue.getDescription());
			pstmtIns.setString(5, issue.getDueDate());
			pstmtIns.setInt(6, issue.getCategoryId());
			pstmtIns.setInt(7, issue.getStatusId());
			pstmtIns.setInt(8, issue.getAssignedToId());
			pstmtIns.setInt(9, issue.getPriorityId());
			pstmtIns.setInt(10 issue.getFixedVersionId());
			pstmtIns.setInt(11, issue.getAuthorId("));
			pstmtIns.setInt(12, issue.getLockVersion());
			pstmtIns.setString(13, issue.getCreatedOn());
			pstmtIns.setString(14, issue.getUpdatedOn());
			pstmtIns.setString(15, issue.getStartDate());
			pstmtIns.setInt(16, issue.getDoneRatio());
			pstmtIns.setFloat(17, issue.getEstimatedHours());
			pstmtIns.setInt(18, issue.getParentId());
			pstmtIns.setInt(19, issue.getRootId());
			pstmtIns.setInt(20, issue.getLft());
			pstmtIns.setInt(21, issue.getRgt());
			pstmtIns.setString(22, issue.getPrivate());
			pstmtIns.setString(23, issue.getClosedOn());
